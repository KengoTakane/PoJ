 ...
 * @return  戻り値: static int              bytesWritten
 * @date    2023/12/15 [0.0.1] 新規作成
 */
/*============================================================================*/
static int write_message(int fd, const mavlink_message_t *message)
...
static void disable_offboard_control(int fd)
{
    ...
    {
        int success = toggle_offboard_control(fd, 0);
...
void* MavlinkMain(void* arg)
{
    ...
    com_timer_init(ENUM_TIMER_MAVM, 1000);

    baudrate = SetupBaudrate(resMavlink);
    if(baudrate == -1)
    {
        dprintf(ERROR, "baudrate = %d. error.", resMavlink->baudrate);
        pthread_exit(NULL); 
    }
    ...
    {
        com_serial_close(fd);
        dprintf(ERROR, "com_shmem_open error. name = %s\n", DEF_MAVLINK_SEND_SHMEM_NAME);
		pthread_exit(NULL);   
    }
