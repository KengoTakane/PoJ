...
class ProcStat:
	...
	def fromByte(self, bytes):
		...
		for i in range(self.num):
			self.stat.append(int.from_bytes(bytes[pos:pos+4], byteorder='little'))
			pos+=4
...
class I2C_BME():
	...
	def fromByte(self, bytes):
		...
		#GasRange
		self.GasRange = int.from_bytes(bytes[pos:pos+4], byteorder='little', signed=False)
		pos += 4
	...
	def toByte(self):
		...
		#
		byte += self.GasRange.to_bytes(4, byteorder='little', signed=False)
