...
/* ECU */
#if 1
	STR_PING_INFO ECUInfo;
	id = com_shmem_open("/ecu", SHM_KIND_PLATFORM);
	if (id == DEF_COM_SHMEM_FALSE) {
		printf("/ecu com_shmem_open() error\n");
		return -1;
	}
	com_shmem_read(id, &ECUInfo, sizeof(ECUInfo));
	printf("ECU connect Status = %d\n", ECUInfo.Stat);
...
/* Dummy */
#if 1
	STR_PING_INFO DummyInfo;
	id = com_shmem_open("/dummy", SHM_KIND_PLATFORM);
	if (id == DEF_COM_SHMEM_FALSE) {
		printf("/dummy com_shmem_open() error\n");
		return -1;
	}
	com_shmem_read(id, &DummyInfo, sizeof(DummyInfo));
	printf("Dummy connect Status = %d\n", DummyInfo.Stat);
